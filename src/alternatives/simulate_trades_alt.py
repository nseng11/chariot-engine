...
def simulate_trade_loops_alt(user_csv_path: str, loop_csv_path: str, output_dir: str, catalog_path: str = None, 
                           ...
                           trade_counter: List[int] = None):
    """
    Alternative implementation of trade simulation focusing on end states.
    Users can rank multiple end states they're interested in, and the system
    optimizes trade execution to maximize user participation.
    ...
    try:
        ...
        # Initialize end states and rankings for each user
        logger.debug("Initializing end states and rankings...")
        user_end_states = {}  # user_id -> list of end state groups
        user_rankings = {}    # user_id -> ranked list of end state groups
        ...
        for loop_idx, loop in loops_df.iterrows():
            ...
            for i in range(1, 4):  # Handle up to 3 users per loop
                ...
                if user_col in loop and pd.notna(loop[user_col]):
                    ...
                    # Create end state key (watch + cash flow) for this specific user
        ...
        # Collect user rankings of end states
        logger.debug("Collecting user rankings...")
        for user_id, end_states in user_end_states.items():
            if not user_status[user_id]['active']:
                continue
                
            # Validate each end state group
            valid_end_states = []
            ...
            # Rank valid end states (for now, using a simple random ranking)
            # In a real implementation, this would be based on user preferences
            random.shuffle(valid_end_states)
            user_rankings[user_id] = valid_end_states
        ...
        # Sort valid loops by number of users (descending)
        valid_loops.sort(key=lambda x: len(end_state_graph[x]), reverse=True)
        ...
        if executed_trades:
            ...
            pd.DataFrame(columns=['loop_id', 'trade_id', 'user_ids', 'from_watches', 'to_watches', 
                                'cash_flows', 'period', 'timestamp']).to_csv(
...
def validate_trade_alt(loop: Dict, user_status: Dict[str, str], users_df: pd.DataFrame) -> Tuple[bool, str]:
    ...
    # For each user, validate their end state (received watch + cash flow)
    for i, user_id in enumerate(loop_users):
        ...
        # Check maximum cash top-up (end state cash flow)
        if cash_flow > user_data['max_cash_top_up']:
            return False, f"User {user_id} end state cash flow {cash_flow:.2f} exceeds maximum top-up {user_data['max_cash_top_up']:.2f}"
